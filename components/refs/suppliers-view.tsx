...
import { archiveSupplier, restoreSupplier } from "@/lib/actions/suppliers";
...
export function SuppliersView({
  ...
  const { active, archived } = useMemo(() => {
    const a: Supplier[] = [];
    const arch: Supplier[] = [];
    for (const s of suppliers) (s.deleted_at ? arch : a).push(s);
  ...
  return (
    <div>
      ...
      <SupplierTable rows={active} onEdit={openEdit} onArchive={setArchiveTarget} />
        ...
        message={
          archiveTarget ? (
            <>
              ...
              supplier, the archive will be blocked with a count.
            </>
          ) : null
        }
  ...
  if (rows.length === 0) {
    return (
      <div
        ...
        style={{ borderColor: "var(--color-line)" }}
      >
        {archived ? "No archived suppliers" : "No suppliers yet"}
      </div>
    );
  }
  return (
    ...
    >
      <table className="w-full border-collapse text-[13px]">
        <thead>
          <tr>
            {["Name", "Country", "Focus", "Default incoterm", "Notes", ""].map(
              (h, i) => (
                <th
                  key={i}
